...
int main()
{
	...
	for(i=0;i<5;i++)//Tüm Ögrencýlerin için  for dongusu
	{
		toplam=0;
		for(j=0;j<4;j++);//Ogrencýlerýn notlarý için for dongusu
		{
			toplam+=ogrenci[i][j];
...
int main()
{
	...
	int *pkurt;//Veri tipi ile ismi atanmýþ deðer.(*p)=Adresini veren kod bloðudur.
	pkurt=&kurt;//adresini istediðim pkurt atamasý eþit eþitir ambersan(&)ý baþta yazcakþekilde &kurt deðerine atadým..
	
	printf("Kurt'un degeri %d:\n",kurt);//10 olarak çýktý yapacaktýr
	printf("Kurt'un Adresi %p:\n",pkurt);//RAM'deki adresini %p veri tipi ile yazdýrdým virgül pkurt ile devam ettim===>0002456485FC57 aynýdeger
	printf("Kurt'un Adresi %p:\n",&kurt);//RAM'deki adresini %p veri tipi ile yazdýrdým virgül &kurt ile devam ettim===>0002456485FC57 aynýdeger
...

{
    ...
    // *bomba=araba;  *bomba nýn öncesi &d olur ise 
